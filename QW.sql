...
BEGIN
	...
	(`ID`)	VALUES
	((SELECT LAST_INSERT_ID() FROM CUOCXE));
END$$
DELIMITER ;
...
BEGIN
	...
	(`ID`,`KhoNguonID`)
	VALUES
	((SELECT last_insert_id() FROM BienBanNhapXuat), KHONGUONID);
END$$
DELIMITER ;
...
BEGIN
	...
	`KhoDichID`)
	VALUES
	((SELECT last_insert_id() FROM BienBanXuat),
	KHODICHID);
...
BEGIN
	...
	`IDNGuoiNhan`)
	VALUES
	((SELECT LAST_INSERT_ID() FROM BienBanNguoiDung),
    PHINOITHANH,
	NGAYGUI,
	IDNGUOIGUI,
	IDNGUOINHAN);
...
BEGIN
	...
	(`ID`)
	VALUES
	((SELECT last_insert_id() FROM KhachHang));
